...
function Navbar() {
  ...
  return (
    <header className="sticky inset-x-0 top-0 z-50 w-full border-b border-slate-800/80 bg-slate-950/85 px-4 shadow-lg shadow-black/20 backdrop-blur-xl">
      <Toaster />
      <nav className="mx-auto flex max-w-7xl items-center py-3">
        ...
        <div className="relative mx-auto hidden w-[50%] max-w-md sm:block">
          <form onSubmit={handleSubmit(onSubmit)} className="relative">
            <input
              ...
              className="w-full rounded-full border border-slate-700/80 bg-slate-900/80 py-2 pl-10 pr-12 text-slate-100 placeholder-slate-500 outline-none hover:border-slate-500 focus:border-cyan-400 focus:ring-4 focus:ring-cyan-400/10 sm:py-3"
              placeholder="Type here..."
            />
            <button
              type="submit"
              className="absolute right-2 top-1/2 -translate-y-1/2 rounded-full bg-cyan-400 p-2 text-slate-950 hover:bg-cyan-300"
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
                fill="none"
        ...
        >
          <span className="block h-[2px] w-full bg-slate-100 group-hover:bg-cyan-300"></span>
          <span className="block h-[2px] w-2/3 bg-slate-100 group-hover:bg-cyan-300"></span>
          <span className="block h-[2px] w-full bg-slate-100 group-hover:bg-cyan-300"></span>
        </button>
        ...
        <div>
          {!authStatus ? (
            <div className="hidden sm:flex mb-8 mt-auto w-full flex-wrap gap-4 px-4 sm:mb-0 sm:mt-0 sm:items-center sm:px-0">
              <Link to="/login">
                <button className="w-full rounded-full border border-slate-700 bg-slate-900 px-4 py-2 text-slate-200 hover:border-cyan-400/50 hover:text-cyan-200 sm:w-auto">
                  Log in
                </button>
              </Link>
              <Link to="/signup">
                <button className="mr-1 w-full rounded-full bg-cyan-400 px-4 py-2 text-center font-bold text-slate-950 shadow-lg shadow-cyan-950/30 hover:bg-cyan-300 active:scale-[0.98] sm:w-auto">
                  Sign up
                </button>
              </Link>
            </div>
            ...
            <button
              className="hidden rounded-full bg-cyan-400 px-4 py-2 font-bold text-slate-950 shadow-lg shadow-cyan-950/30 hover:bg-cyan-300 active:scale-[0.98] sm:block"
          ...
          {isOpen && (
            <div
              className={`fixed inset-y-0 right-0 flex w-full max-w-xs shrink-0 translate-x-0 flex-col border-l border-slate-800 bg-slate-950/95 shadow-2xl shadow-black/50 backdrop-blur-xl sm:static sm:ml-4 sm:w-auto sm:border-none`}
            >
              <div className="relative flex w-full items-center justify-between border-b border-slate-800 px-4 py-3 sm:hidden">
              ...
              <ul className="my-4 flex w-full flex-wrap gap-2 px-4 sm:hidden">
                <Link
                  ...
                  className="w-full"
                >
                  <button className="flex w-full items-center justify-start gap-x-4 rounded-xl border border-slate-800 bg-slate-900/60 px-4 py-2 text-left text-slate-200 hover:border-cyan-400/50 hover:bg-cyan-400/10 hover:text-cyan-200 focus:border-cyan-400 focus:bg-cyan-400/10 focus:text-cyan-200">
              ...
              <div className="flex absolute bottom-5">
                {!authStatus ? (
                  ...
                  <button
                    className="ml-4 w-full rounded-full border border-slate-700 bg-slate-900 px-4 py-2 text-slate-200 hover:border-cyan-400/50 hover:text-cyan-200 sm:w-auto"
                    onClick={handleLogout}
                  >
                    Logout
                  </button>
